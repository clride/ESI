...
public class Interpreter
{
    ...
    public static void ExecuteProgram(string code, int memorySize)
    {
        ExecuteProgram(code, memorySize, out _, out _);
    }
    ...
    /// <param name="code">Source code string.</param>
    /// <param name="memorySize">Size of RAM memory (in cells).</param>
    /// <param name="memoryDump">The memory state of the program after it has finished executing.</param>
    /// <param name="frameBufferDump">The framebuffer state of the program after it has finished executing.</param>>
    public static void ExecuteProgram(string code, int memorySize, 
        out int[] memoryDump, out int[,] frameBufferDump)
    {
        int memorySizeBytes = memorySize * 4;
        int[] memory = new int[memorySize];
        var currentAddress = 0;

        bool editingFrameBuffer = false;

        InitializeFrameBuffer(out int[,] frameBuffer);
        ...
        do
        {
            ...
            else
            {
                if (instruction == '[')
                {
                    if (memoryVal == 0) pc = FindMatchingBracket(code, pc);
                }
